...
def _calculate_info_score(extracted_vars: dict) -> float:
...
def _generate_clarifying_question(context: dict) -> str:
    variable_map = {'revisit_indicator': '재방문 여부', 'visit_local_indicator': '지방 방문 계획', 'stay_duration': '여행 기간', 'planned_activity': '주요 활동'}
    missing_vars = []
    if VARIABLE_WEIGHTS:
    ...
    question_prompt = f"당신은 친절한 여행 플래너입니다. 사용자 맞춤 추천을 위해 현재 '{', '.join(missing_vars[:2])}' 정보가 부족합니다. 이 정보를 자연스럽게 물어보는 질문을 한 문장으로 만들어주세요."
    try:
...
def get_user_cluster(user_query: str, previous_context: dict = None):
    ...
    if score > 0.50:
        ...
        question = _generate_clarifying_question(current_context)
